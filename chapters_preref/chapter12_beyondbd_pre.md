...
The most successful models to tackle this question have been models of protracted speciation [@Lambert2015-vb, @Etienne2012-uh, @Rosindell2010-dk]. In such models, speciation begins by the formation of an incipient species. This represents a “partial” species; one can imagine, for example, that this is a population that has split off from the main range of the species, but has not yet evolved full reproductive isolation. The incipient species only becomes a “full” species if it survives some time interval, tau, that represents the time it takes to evolve full reproductive isolation (Figure xxx).
...
Likelihood approaches are available for this model of protracted speciation. Again, the likelihood must be calculated using numerical methods [@Lambert2015-vb]. XXX example.
