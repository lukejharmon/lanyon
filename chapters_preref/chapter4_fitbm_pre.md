...
One simple example is based on Newton’s method of optimization [as implemented, for example, by the r function nlm()]. We can use this algorithm to quickly find accurate ML estimates<sup><a name="footnote4.2_back">[2](#footnote4.2)</a></sup>.
...
This is the ratio of the probability of drawing the parameter values $p$ and $p’$ from the prior. Since our priors are uniform, this is always 1.

b.	The proposal density ratio, $R_{proposal}$.

This is the ratio of probability of proposals going from $p$ to $p’$ and the reverse. We have already declared a symmetrical proposal density, so that $Q(p'|p) = Q(p|p')$ and $R_{proposal} = 1$.
